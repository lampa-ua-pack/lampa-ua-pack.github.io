...
export const DEFAULT_RELAY = 'wss://itty.ws/c/';
...
export async function roomChannel(roomId, password) {
  const hex = await sha256hex((roomId || '').toUpperCase() + '|' + (password || ''));
...
export function syncDecision(diff, t = TUNING) {
  const abs = Math.abs(diff);
  if (abs > t.hardSeekS) return { action: 'seek', rate: 1 };
  if (abs > t.toleranceS) {
    const offset = Math.max(-t.maxRateOffset, Math.min(t.maxRateOffset, diff * t.rateGain));
...
class Sock {
  constructor({ relay, channel, alias = '', echo = false, reconnect = false, onEvent = () => {} }) {
    this.relay = relay;
  ...
  open() {
    ...
    this.ws.onmessage = (e) => {
      ...
      if (d.type === 'join') {
        if (d.self) {
          ...
          for (const u of d.users || []) {
            if (!this.findMember(u.uid)) this.members.push({ uid: u.uid, alias: u.alias || '' });
          }
          // The relay does not always list us among the users; host election counts
          // members, so we have to be in our own list.
          if (!this.findMember(this.uid)) this.members.push({ uid: this.uid, alias: this.alias });
          this.onEvent({ kind: 'ready', date: d.date, total: d.total, members: this.members });
        } else {
          if (!this.findMember(d.uid)) this.members.push({ uid: d.uid, alias: d.alias || '' });
          this.onEvent({ kind: 'join', date: d.date, uid: d.uid, alias: d.alias || '', total: d.total });
        }
        return;
      }
      ...
      if (d.type === 'leave') {
        ...
        this.onEvent({ kind: 'leave', date: d.date, uid: d.uid, alias: d.alias || '', total: d.total });
        return;
      }
  ...
  scheduleRetry() {
    ...
    this.retryTimer = setTimeout(() => { this.retryTimer = null; this.open(); }, TUNING.reconnectMs);
  }

  findMember(uid) { return this.members.find((m) => m.uid === uid) || null; }
...
const emptyMeta = () => ({ title: '', poster: '', url: '', tmdb_id: 0, source: '', type: '' });
...
export class Party {
   ...
   */
  constructor({ video, relay = DEFAULT_RELAY, name = '', publish = true, onEvent = () => {} }) {
    this.video = video;
    this.name = name;
    ...
    this.tick = setInterval(() => this.#holdTick(), 1000);
  }
  ...
  displayName() { return (this.name || '').trim() || this.pid; }
  serverNow() { return Date.now() + this.serverTimeOffset; }
  iAmHost() { return !!this.owner && this.owner === this.pid; }
  memberCount() { return this.room ? this.room.members.length : 0; }

  emit(type, payload) { this.onEvent(type, payload); }

  /** Snapshot for the debug readout. */
  debug() {
    const best = this.pingSamples.length ? Math.min(...this.pingSamples.map((s) => s.rtt)) : null;
    return {
      ws: this.room && this.room.ws ? ['CONNECTING', 'OPEN', 'CLOSING', 'CLOSED'][this.room.ws.readyState] : 'NO_WS',
      room: this.roomId, host: this.owner, iAmHost: this.iAmHost(),
      members: this.memberCount(), offset: Math.round(this.serverTimeOffset),
      rtt: this.pingSamples.length ? this.pingSamples[this.pingSamples.length - 1].rtt : null,
      bestRtt: best, pending: Object.keys(this.echoPending).length,
    ...
    this.watchdogTimer = setInterval(() => {
      ...
      for (const key of Object.keys(this.echoPending)) {
        if (now - this.echoPending[key] > TUNING.echoTimeoutMs) {
          this.echoPending = {};
          try { this.room.ws.close(); } catch (err) {}
          return;
        }
      }
    }, 5000);
  ...
  #ad() {
    return {
      ...
      title: this.meta.title || '', poster: this.meta.poster || '',
      owner: this.displayName(), members: this.memberCount(),
      pwd: this.password ? 1 : 0, tmdb: this.meta.tmdb_id || 0,
  ...
  #startLobbyAgent() {
    ...
    this.lobbyHost = new Sock({
      relay: this.relay, channel: LOBBY_CHANNEL, echo: false, reconnect: true,
      onEvent: (e) => {
        ...
        setTimeout(() => {
          if (this.lobbyHost && this.inRoom) this.lobbyHost.send({ t: 'ad', r: this.#ad() });
  ...
  #stopLobbyAgent() {
    if (this.lobbyHost) { this.lobbyHost.close(); this.lobbyHost = null; }
  }
  ...
  browse() {
    return new Promise((resolve) => {
      ...
      const finish = () => {
        ...
        clearTimeout(timer);
        probe.close();
        resolve(Object.values(found).sort((a, b) => (b.members || 0) - (a.members || 0)));
      };

      const probe = new Sock({
        relay: this.relay, channel: LOBBY_CHANNEL, echo: false, reconnect: false,
        onEvent: (e) => {
          if (e.kind === 'ready') {
            probe.send({ t: 'who' });
  ...
  async #connect(roomId, password, onReady) {
    ...
    this.room = new Sock({
      relay: this.relay, channel, alias: this.displayName(),
      echo: true, reconnect: true,
  ...
  async join(roomId, password, fallbackName) {
    ...
    await this.#connect(roomId, password, (ev) => {
      ...
      if (ev.total <= 1) { this.#failJoin(); return; }
  ...
  async create({ url, title = '', name = '', password = '', poster = '', tmdbId = 0, source = '', type = 'movie' }) {
    if (this.createPending) return null;
    if (this.inRoom) { this.emit('error', { key: 'already_in_room', name: this.roomName || this.roomId }); return null; }
    if (!url) { this.emit('error', { key: 'need_url' }); return null; }
    ...
    await this.#connect(id, password, () => {
      ...
      this.emit('stream', { url: this.meta.url, title: this.meta.title || this.roomName, poster: this.meta.poster });
  ...
  #reset() {
    this.#clearRate();
    clearTimeout(this.expectedSeekTimer); this.expectedSeekTimer = null;
    clearTimeout(this.enforceTimer); this.enforceTimer = null;
    clearTimeout(this.joinTimer); this.joinTimer = null;
    clearTimeout(this.createTimer); this.createTimer = null;
  ...
  info() {
    return {
      id: this.roomId, name: this.roomName, owner: this.owner,
      iAmHost: this.iAmHost(), members: this.memberCount(), meta: this.meta,
  ...
  setStream(url, title) {
    ...
    this.emit('stream', { url, title: this.meta.title || this.roomName, poster: this.meta.poster });
  }
  ...
  #bufferedAhead(position) {
    ...
    for (let i = 0; i < ranges.length; i++) {
      if (position >= ranges.start(i) - 1 && position <= ranges.end(i)) return ranges.end(i) - position;
    }
    return OUT_OF_RANGE;
  }
  ...
  #holdReadyNow() {
    ...
    if (ahead >= TUNING.holdMinBufferS && idle > TUNING.holdStallMs) return true;

    // Some players stall until poked; a tiny seek restarts the fill.
    if (!this.holdNudgeDone && idle > TUNING.holdNudgeMs && vid.readyState >= 1) {
  ...
  #startJoinHold(newcomerPid) {
    ...
    if (!this.holdActive) {
      ...
      if (vid && !vid.paused) { this.#expectPause(); vid.pause(); }
      this.emit('notice', { key: 'notice_hold' });
    }

    if (newcomerPid && newcomerPid !== this.pid) this.holdWaiting[newcomerPid] = true;
  ...
  #releaseHold(position) {
    ...
    if (vid.paused) {
      this.#expectPlay();
      const p = vid.play();
      if (p && p.catch) p.catch(() => { this.expected.play = false; });
    }
  }
  ...
  #applyHold(position) {
    ...
    if (!vid.paused) { this.#expectPause(); vid.pause(); }
  ...
  #holdTick() {
    if (!this.holdActive) return;
    if (this.iAmHost()) { this.#checkHoldDone(); return; }
  ...
  #isRewinding() { return Date.now() < this.rewindUntil; }
  ...
  #onRoomEvent(e) {
    ...
    if (e.kind === 'close' || e.kind === 'join') { this.emit('members', this.memberCount()); return; }
    if (e.kind === 'error') { this.emit('error', { key: 'relay_error', text: e.text }); return; }
    if (e.kind === 'leave') { this.#onMemberLeave(e); return; }
  ...
  #handleMessage(m, e) {
    ...
    if (m.t === 'hello') {
      ...
      const reconnected = seenAt && (Date.now() - seenAt) < TUNING.reconnectHelloMs;
    ...
    if (m.t === 'ready') { this.#markHoldReady(m.u); return; }
    ...
    if (m.t === 'me') { this.emit('members', this.memberCount()); return; }

    if (m.t === 'state') { if (this.joining) this.#acceptRoomState(m, e.date); return; }
    ...
    if (m.t === 'url') {
      ...
      this.emit('stream', { url: m.url, title: m.ti || this.roomName, poster: this.meta.poster });
      return;
    }
    ...
    if (m.t === 'sync' || m.t === 'act') {
      if (!this.inRoom) return;
      if (m.t === 'act' && m.v) {
        const key = { resumed: 'act_resumed', playing: 'act_resumed', paused: 'act_paused', seeked: 'act_seeked' }[m.v];
      ...
      if (m.t === 'sync' && (this.iAmHost() || (this.owner && m.u !== this.owner))) return;
      ...
      setTimeout(() => { this.isSystemSyncing = false; }, TUNING.systemSyncMs);
      return;
    }

    if (m.t === 'bye') { this.emit('members', this.memberCount()); }
  }

  #sendHostState() {
    this.send({
      t: 'state',
      rn: this.roomName, own: this.owner || this.pid,
      url: this.meta.url, ti: this.meta.title, po: this.meta.poster,
      tm: this.meta.tmdb_id, src: this.meta.source, ty: this.meta.type,
  ...
  #acceptRoomState(msg, atServerTime) {
    ...
    this.meta = {
      title: msg.ti || '', poster: msg.po || '', url: msg.url,
      tmdb_id: msg.tm || 0, source: msg.src || '', type: msg.ty || 'movie',
    };
    ...
    this.targetInitialState = needsInitialSync ? { state, position, atServerTime } : null;
  ...
  #onMemberLeave(e) {
    ...
    if (leaverPid && this.holdWaiting[leaverPid]) this.#markHoldReady(leaverPid);
    ...
    setTimeout(() => {
      ...
      const uids = [this.room.uid, ...this.room.members.map((m) => m.uid).filter((u) => u !== this.room.uid)].sort();
  ...
  #sendSync(state, verb) {
    ...
    if (verb) this.send({ t: 'act', s: state, p: vid.currentTime || 0, v: verb, n: this.displayName() });
    else this.send({ t: 'sync', s: state, p: vid.currentTime || 0 });
  }

  /** Only the host beats, and it reports pause as well — guests just follow. */
  #heartbeat() {
    if (!this.inRoom || this.initialSyncLock || this.isSystemSyncing || this.holdActive) return;
  ...
  #expectPlay() {
    ...
    setTimeout(() => { this.expected.play = false; }, TUNING.expectPlayMs);
  }

  #expectPause() {
    this.expected.pause = true;
    setTimeout(() => { this.expected.pause = false; }, TUNING.expectPlayMs);
  }
  ...
  #applySync(state, basePosition, atServerTime) {
    ...
    if (busy) {
      ...
      const expected = expectedPositionNow(state, basePosition, atServerTime, this.serverNow());
    ...
    if (state === 'playing' && vid.paused && !busy) {
      this.#expectPlay();
      const p = vid.play();
      if (p && p.catch) p.catch(() => { this.expected.play = false; this.emit('error', { key: 'autoplay_blocked' }); });
    }
  }

  #videoBusy() {
    const vid = this.video;
    return !!this.buffering || !vid || vid.readyState < 3;
  }
    ...
    const expected = expectedPositionNow(state, position, atServerTime, this.serverNow());
    ...
    if (state === 'paused') {
      ...
      this.#expectPlay();
      const p = vid.play();
      if (p && p.catch) p.catch(() => this.emit('error', { key: 'autoplay_blocked' }));
    }
  ...
  #hookVideo() {
    ...
    vid.addEventListener('waiting', () => { this.buffering = true; });
    vid.addEventListener('canplay', () => { this.buffering = false; });
    vid.addEventListener('playing', () => { this.buffering = false; });

    vid.addEventListener('play', () => {
      if (this.initialSyncLock) {
        if (this.targetInitialState && this.targetInitialState.state === 'paused') vid.pause();
        return;
      }
      const wasExpected = this.expected.play;
      this.expected.play = false;
      if (wasExpected) return;
      if (this.bufferPaused) { this.bufferPaused = false; return; }
      if (this.#isRewinding()) { this.lastUserActionTime = Date.now(); return; }
      this.lastUserActionTime = Date.now();
    ...
    vid.addEventListener('pause', () => {
      ...
      this.expected.pause = false;
      if (wasExpected) return;
      if (this.buffering || vid.readyState < 3) { this.bufferPaused = true; return; }
      if (this.#isRewinding()) { this.lastUserActionTime = Date.now(); return; }
      this.lastUserActionTime = Date.now();
      this.#sendSync('paused', 'paused');
    });

    // Lampa learns about scrubbing from its own rewind event; on the web the closest
    // signal is a seek we did not start ourselves.
    vid.addEventListener('seeking', () => {
      if (this.isSystemSyncing || this.holdActive || this.expected.seek !== -1) return;
      this.#onUserRewind();
    });
    ...
    vid.addEventListener('seeked', () => {
      ...
      if (this.expected.seek !== -1) {
        // Landing on the nearest keyframe counts as arrival; re-forcing the position
        // from here used to bounce the player between two keyframes.
        const wasExpectedSeek = Math.abs(vid.currentTime - this.expected.seek) < 1.5;
        this.#clearExpectedSeek();
        if (wasExpectedSeek) return;
      }

      // Still scrubbing: extend the grace window instead of broadcasting every step.
      if (this.#isRewinding()) { this.rewindUntil = Date.now() + 400; return; }
