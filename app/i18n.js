...
const DICT = {
  uk: {
    ...
    empty_hint: 'Кімната з’явиться тут, щойно хтось у Lampa створить її з увімкненою публікацією. Якщо код уже є — введіть його вище.',
    ...
    note: 'Сторінка працює за протоколом плагіна lparty.js для Lampa і підключається до тих самих кімнат. Потік має віддаватися по https і з заголовком Access-Control-Allow-Origin, інакше браузер його не пустить.',
    ...
    viewers: { one: '{n} глядач', few: '{n} глядачі', many: '{n} глядачів', other: '{n} глядача' },
    ...
    mixed_content: 'Потік віддається по http, а сторінка по https — браузер це блокує',
    cors_error: 'Потік не завантажується: схоже, сервер не віддає Access-Control-Allow-Origin',
    playback_error: 'Помилка відтворення: {details}',
    media_error: 'Браузер не зміг відкрити потік (формат, CORS або mixed content)',
  },
  ...
  en: {
    ...
    empty_hint: 'A room appears here as soon as someone in Lampa creates one with publishing enabled. If you already have a code, enter it above.',
    ...
    note: 'This page speaks the protocol of the lparty.js plugin for Lampa and joins the same rooms. The stream must be served over https with an Access-Control-Allow-Origin header, otherwise the browser will not load it.',
    ...
    mixed_content: 'The stream is served over http while this page is https — the browser blocks that',
    cors_error: 'The stream will not load: the server likely omits Access-Control-Allow-Origin',
    playback_error: 'Playback error: {details}',
    media_error: 'The browser could not open the stream (format, CORS or mixed content)',
  },
  ...
  ru: {
    ...
    empty_hint: 'Комната появится здесь, как только кто-то в Lampa создаст её с включённой публикацией. Если код уже есть — введите его выше.',
    ...
    note: 'Страница работает по протоколу плагина lparty.js для Lampa и подключается к тем же комнатам. Поток должен отдаваться по https и с заголовком Access-Control-Allow-Origin, иначе браузер его не пустит.',
    ...
    viewers: { one: '{n} зритель', few: '{n} зрителя', many: '{n} зрителей', other: '{n} зрителя' },
    ...
    mixed_content: 'Поток отдаётся по http, а страница по https — браузер это блокирует',
    cors_error: 'Поток не загружается: похоже, сервер не отдаёт Access-Control-Allow-Origin',
    playback_error: 'Ошибка воспроизведения: {details}',
    media_error: 'Браузер не смог открыть поток (формат, CORS или mixed content)',
  },
};
...
export function t(key, params) {
  ...
  return params
    ? value.replace(/\{(\w+)\}/g, (m, name) => (params[name] === undefined ? m : params[name]))
    : value;
}
